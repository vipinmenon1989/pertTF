...
from train_data_gen import prepare_data,prepare_dataloader
...
def eval_testdata(
    ...
    return results
def save_metric_curve(metric_name, train_vals, valid_vals, epochs, save_dir):
    plt.figure()
    plt.plot(epochs, train_vals, label=f"train/{metric_name}")
    plt.plot(epochs, valid_vals, label=f"valid/{metric_name}")
    plt.xlabel("epoch")
    plt.ylabel(metric_name)
    plt.title(f"{metric_name} curve")
    plt.legend()
    plt.tight_layout()
    plt.savefig(save_dir / f"{metric_name}_curve.png", dpi=150)
    plt.close()
...
def wrapper_train(model, config, data_gen,
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if epoch % 2 == 1:
            ...
            save_dir2=save_dir / f'epoch_{epoch:02d}_imgs'
            ...
            for eval_dict_key, eval_adata in eval_adata_dict.items():
                ...
                for res_key, res_img_val in results.items():
                    if res_key in save_image_types:
                        filename = f"{dynamic_eval_key}_{res_key}_fold{fold}_epoch{epoch}.png"
                        res_img_val.savefig(save_dir2 / filename, dpi=300, bbox_inches='tight')
                        metrics_to_log[f"test/{dynamic_eval_key}_{res_key}"] = wandb.Image(
                            str(save_dir2 / filename),
                            caption=f"{dynamic_eval_key}_{res_key} | fold {fold}, epoch {epoch}",
                        )
                        figure_logs[f"valid/fold{fold}/{res_key}_epoch{epoch}"]=wandb.Image(str(save_dir2/filename),caption=f"{res_key} | fold {fold} epoch {epoch}")
                if figure_logs:
                ...
                if config.ps_weight > 0:
                    ...
                    for lon_c in loness_columns:
                        ...
                        fig_lonc.savefig(save_dir2 / f"{eval_dict_key}_loness_{lon_c_rep}_fold{fold}_e{epoch}.png", dpi=300,bbox_inches='tight')
                    if ('ps_names' in data_gen) & ('ps_pred' in adata_ret.obsm) :
                        ...
                        for si_i in range(len(predicted_ps_names)):
                            ...
                            fig_lonc_pred = sc.pl.umap(adata_ret,color=[f'{lon_c_rep}_pred'],title=[f"loness {lon_c_rep}_pred  e{epoch}",],
                                frameon=False,return_fig=True, show=False,palette="tab20",)
                            plt.close()
                            fig_lonc_pred.savefig(save_dir2 / f"{eval_dict_key}_loness_{lon_c_rep}_fold{fold}_pred_e{epoch}.png", dpi=300,bbox_inches='tight')
                    results["adata"] = adata_ret
                if "adata" in results:
                    results["adata"].write_h5ad(save_dir / f'adata_last_validation_{eval_dict_key}_fold{fold}.h5ad')
