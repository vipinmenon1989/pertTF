...
class PerturbationTFModel(TransformerModel):
    def __init__(self, n_pert: int, nlayers_pert: int, *args, **kwargs):
        ...
        self.batch2_pad_id = (
            None  # kwargs.get("batch2_pad_id") if "batch2_pad_id" in kwargs else 2
        )
        # self.batch2_encoder = nn.Embedding(2, d_model, padding_idx=self.batch2_pad_id)
        self.batch2_encoder = Batch2LabelEncoder(
            2, d_model
        )  # should replace 2 to n_batch later
        self.n_pert = n_pert
    ...
    def encode_batch_with_perturb(
        ...
        shape_perts = (
            (N, self.n_pert) if time_step is not None else (N, src.size(1), self.n_pert)
        )
        pert_outputs = array_func(shape_perts, dtype=float32_)

        # add for cls predictions
        shape_cls = (
            (N, self.n_cls) if time_step is not None else (N, src.size(1), self.n_cls)
        )
        ...
        for i in trange(0, N, batch_size):
            ...
            pert_labels_d = (
                pert_labels[i : i + batch_size].to(device)
                if pert_labels is not None
                else None
            )
