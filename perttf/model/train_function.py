...
def eval_testdata(
    ...
    celltypes_indexes = np.array(
        [cell_type_to_index[cell_type] for cell_type in celltypes_labels]
    )
    ...
    perturbation_indexes = np.array(
        [
            genotype_to_index[perturbation_type]
            for perturbation_type in perturbation_labels
        ]
    )
    ...
    if "cls" in include_types:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=config.amp):
            ...
            #    return_np=True,
            # )
            cell_embeddings, pert_preds, cls_preds = model.encode_batch_with_perturb(
                all_gene_ids,
                all_values.float(),
                src_key_padding_mask=src_key_padding_mask,
                batch_size=config.batch_size,
        ...
        X_pert_pred_probs = np.exp(pert_preds) / np.sum(
        ...
        X_pert_cls_probs = np.exp(cls_preds) / np.sum(
...
def wrapper_train(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_data_pt, valid_data_pt = prepare_data(
            data_gen, config, sort_seq_batch=config.per_seq_batch_sample, epoch=epoch
        )
        ...
        val_loss, val_loss_next, val_mre, val_mre_next, val_dab, val_cls, val_pert = (
            evaluate(
                model,
                loader=valid_loader,
                config=config,
        ...
        if epoch % 2 == 1:
            ...
            torch.save(
                best_model.state_dict(), save_dir / f"model_e{best_model_epoch}.pt"
            )
            ...
            for eval_dict_key, eval_adata in eval_adata_dict.items():
                ...
                for res_key, res_img_val in results.items():
                    if res_key in save_image_types:
                        res_img_val.savefig(
                            save_dir
                            / f"{eval_dict_key}_embeddings_{res_key}_e{epoch}.png",
                            dpi=300,
                            bbox_inches="tight",
                        )
                ...
                if "adata" in results:
                    results["adata"].write_h5ad(
                        save_dir / f"adata_last_validation_{eval_dict_key}.h5ad"
                    )
